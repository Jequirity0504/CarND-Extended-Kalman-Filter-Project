...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  //then transform it from microseconds to seconds.
  ...
  // update state transition matrix F so that the time is integrated
  ekf_.F_(0,2) = dt;
  ekf_.F_(1,3) = dt;

  //set the process covariance matrix Q based on new elapsed time
  ekf_.Q_ = MatrixXd(4,4);
